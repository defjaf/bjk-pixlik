"""
Apply BJK98 pixel-space likelihood to Euclid TR1 data with EB.

Estimates EE, BB, and EB bandpowers jointly via Newton-Raphson for the TR1
southern-patch shear map (dec<0, tom_bin_id>0, corrected shear), NSIDE=128,
Delta_ell=10 -- matches the Almanac TR1_nside128_dl10_v{1,2,3,4} runs
(see Euclid-Almanac/TR1/runs/RUN_LOG.md).

Run from repo root:
    python3 examples/run_bjk_TR1_dl10.py

NOTE: prepared per user request but NOT to be run until after the Almanac
chains complete (2026-07-31).
"""
...
DATA_FITS = os.path.join(TR1_DIR, 'maps', f'TR1_shear_nside{NSIDE}.fits')
NINV_FITS = os.path.join(TR1_DIR, 'maps', f'TR1_weights_nside{NSIDE}.fits')

OUT_DIR = os.path.join(TR1_DIR, 'runs', 'bjk_results')
os.makedirs(OUT_DIR, exist_ok=True)
OUT_DAT = os.path.join(OUT_DIR, f'bjk_TR1_nside{NSIDE}_dl10_eb.dat')
OUT_PNG = os.path.join(OUT_DIR, f'bjk_TR1_nside{NSIDE}_dl10_eb.png')
...
def main():
    print("="*70)
    print("BJK98 Pixel Likelihood: Euclid TR1 EE+BB+EB, Delta_ell=10 (n_P=1)")
    print("="*70)
    ...
    print(f"--- ML bandpowers (Euclid TR1, NSIDE={NSIDE}, Delta_ell=10) ---")
    ...
    for ax, spec in zip(axes, ('EE', 'BB', 'EB')):
        ...
        ax.set_title(rf'Euclid TR1, NSIDE={NSIDE} -- {spec}')
        ax.legend()
        ax.grid(True, alpha=0.3)

    fig.suptitle('BJK98 pixel-space ML bandpowers (EE+BB+EB) -- Euclid TR1 data',
                 y=1.01, fontsize=14)
    fig.tight_layout()
